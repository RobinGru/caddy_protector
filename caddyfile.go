// Package caddyprotector provides a Cap-protected HTTP middleware module for Caddy.
...
func (bb *CaddyProtector) UnmarshalCaddyfile(d *caddyfile.Dispenser) error {
	for d.Next() {
		for d.NextBlock(0) {
			if err := bb.parseCaddyfileDirective(d); err != nil {
				return err
			}
		}
	}
...
func (bb *CaddyProtector) parseCaddyfileDirective(d *caddyfile.Dispenser) error {
	param := d.Val()
	switch param {
	case "disable_csp_header":
		if d.CountRemainingArgs() != 0 {
			return d.ArgErr()
		}
		bb.DisableCSPHeader = true
		return nil
	case "whitelist_country", "blacklist_country":
		...
		return nil
	case "deny_header_substring":
		args := d.RemainingArgs()
		if len(args) != 2 {
			return d.Errf("deny_header_substring erwartet genau 2 Argumente: <header-name> <value>")
		}
		bb.DenyHeaderSubstrings = append(bb.DenyHeaderSubstrings, HeaderSubstringRule{Name: args[0], Needle: args[1]})
		return nil
	}
	...
	return bb.setCaddyfileOption(d, param, arg)
}

func (bb *CaddyProtector) setCaddyfileOption(d *caddyfile.Dispenser, param, arg string) error {
	switch param {
	case "allow_for":
